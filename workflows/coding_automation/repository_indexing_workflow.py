...
async def index_repository(
    repo_path: str, input_data: dict[str, Any]
) -> dict[str, Any]:
    ...
    try:
        ...
        # Get file list
        result = subprocess.run(
            ["git", "ls-files"], cwd=repo_path_obj, capture_output=True, text=True
        )
...
async def save_to_database(repo_index: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(
            f"Successfully saved repository to database: {repo_index['repository_id']}"
        )
...
class RepositoryIndexingWorkflow:
    ...
    async def run(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        workflow.logger.info(
            f"Starting repository indexing for {input_data.get('owner', 'unknown')}/{input_data.get('name', 'unknown')}"
        )
        ...
        try:
            ...
            result = {
                ...
                "execution_time_ms": execution_time_ms,
                "files_processed": (
                    repository_index["file_count"] if repository_index else 0
                ),
                "files_skipped": 0,
            }
            ...
            if repo_path:
                try:
                    ...
                    workflow.logger.warning(
                        f"Failed to cleanup repository: {cleanup_error}"
                    )
