...
class GitRepository:
    ...
    async def clone(self, remote_url: str, credentials: Optional[RepositoryCredentials] = None, branch: str = "main"):
        """Clone a repository."""
        ...
        if credentials:
            if credentials.token:
                ...
                if remote_url.startswith("https://"):
                    clone_url = remote_url.replace("https://", f"https://{credentials.token}@")
            elif credentials.username and credentials.password:
                # For HTTPS with username/password
                if remote_url.startswith("https://"):
                    clone_url = remote_url.replace("https://", f"https://{credentials.username}:{credentials.password}@")
                ...
                if credentials.ssh_key_path:
                    ...
                    ssh_key_file = self.repo_path.parent / f"ssh_key_{hashlib.md5(credentials.ssh_key_content.encode()).hexdigest()}"
...
@activity.defn
async def index_repository(repo_path: str, repository_info: RepositoryInfo) -> RepositoryIndex:
    """
    ...
    try:
        ...
        activity.logger.info(f"Successfully indexed repository: {len(valid_files)} files, {total_lines} lines")
        return repo_index
...
async def cleanup_repository(repo_path: str) -> None:
    ...
    try:
        ...
        if repo_path_obj.exists():
            ...
            activity.logger.info(f"Successfully cleaned up repository")
