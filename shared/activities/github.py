...
async def get_pull_request_details(input: PullRequestInfo) -> PullRequestDetails:
    ...
    """
    activity.logger.info(f"Getting details for PR #{input.pr_number} in {input.repository}")
...
async def get_diff_content(input: PullRequestInfo) -> DiffContent:
    ...
    """
    activity.logger.info(f"Getting diff for PR #{input.pr_number} in {input.repository}")
